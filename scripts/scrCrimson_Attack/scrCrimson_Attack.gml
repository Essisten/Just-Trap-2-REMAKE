/// @description Big Just: Crimson phase
function scrCrimson_Attack() {
	if (tmpHP > 10)
	{
	   var f;
	   f = instance_create(v_w, __view_get( e__VW.YView, 0 ) + (__view_get( e__VW.HView, 0 ) / 2), objCherry);
	   f.direction = point_direction(f.x, f.y, objPlayer.x, objPlayer.y);
	   f.speed = fruit_spd;
	}
	if (tmpHP > 20)
	{
	   var yy = random_range(__view_get( e__VW.YView, 0 ), v_h);
	...
	{
	   var xx = objPlayer.x;
	   f[0] = instance_create(__view_get( e__VW.XView, 0 ), __view_get( e__VW.YView, 0 ), objCherry);
	   f[1] = instance_create(__view_get( e__VW.XView, 0 ), v_h, objCherry);
	   if (global.difficulty == 3)
	   {
	       f[2] = instance_create(v_w, __view_get( e__VW.YView, 0 ), objCherry);
	   ...
	   for (var i = 0; i < array_length_1d(f); i++)
	   {
	       ...
	       f[i].direction = point_direction(f[i].x, f[i].y, xx, __view_get( e__VW.YView, 0 ) + __view_get( e__VW.HView, 0 ) / 2);
	       f[i].image_blend = c_blue;
	   }
	}
	if (tmpHP > 40)
	{
	       g[0] = instance_create(__view_get( e__VW.XView, 0 ), random_range(256, 448), objCherry);
	       ...
	       if (global.difficulty == 3)
	       {
	           ...
	           g[3] = instance_create(random_range(160, 416), __view_get( e__VW.YView, 0 ), objCherry);
