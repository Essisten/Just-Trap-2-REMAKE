/// @description Big Just: Gay phase
function scrGay_Attack() {
	if (tmpHP > 10)
	{
	...
	if (tmpHP > 40)
	{
	   with (instance_create(__view_get( e__VW.XView, 0 ) + __view_get( e__VW.WView, 0 ), random_range(256, 448), objCherry))
	   {
	...
	if (tmpHP > 50)
	{
	   ...
	   {
	       with (instance_create(random_range(160, 416), __view_get( e__VW.YView, 0 ), objCherry))
	       {
	...
	if (tmpHP > 60)
	{
	    with (instance_create(v_w, __view_get( e__VW.YView, 0 ) + 32, objGayTornado))
	    {
	        hspeed = -4;
	    }
	}
