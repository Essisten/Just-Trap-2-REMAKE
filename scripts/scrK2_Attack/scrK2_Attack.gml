/// @description Kamilia 2 phase
function scrK2_Attack() {
	if (HP <= 10 and objK2_Face.image_index == 0)
	{
	    frozen = true;
	    instance_create(__view_get( e__VW.XView, 0 ) + __view_get( e__VW.WView, 0 ) / 2,
	      __view_get( e__VW.YView, 0 ) + __view_get( e__VW.HView, 0 ) / 2, objK2_LightEffect);
	    with (objK2_Face)
	    {
	        alarm[1] = 50;
	        image_index = 1;
	...
	if (tmpHP > 40)
	{
	    for (var i = 64; i < __view_get( e__VW.XView, 0 ) + __view_get( e__VW.WView, 0 ) - 128; i += 96 - (global.difficulty * 16))
	    {
	        instance_create(i, __view_get( e__VW.YView, 0 ) - 8, objK2_Bomb);
	    }
	}
