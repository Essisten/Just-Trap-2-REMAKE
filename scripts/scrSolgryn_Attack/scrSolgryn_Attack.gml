/// @description Big Just: Solgryn phase
function scrSolgryn_Attack() {
	if (tmpHP > 10)
	{
	...
	if (tmpHP > 20)
	{
	   ...
	   for (; i <= 800 - 64; i += step)
	   {
	       var h = instance_create(__view_get( e__VW.XView, 0 ) + i, __view_get( e__VW.YView, 0 ), objCherryReflectable);
	...
	if (tmpHP > 30)
	{
	   instance_create(random_range(160, 416), __view_get( e__VW.YView, 0 ), objKitty);
	}
	if (tmpHP > 40)
	{
	...
	if (HP <= 0 and !instance_exists(objSolgryn_Spider))
	{
	    frozen = true;
